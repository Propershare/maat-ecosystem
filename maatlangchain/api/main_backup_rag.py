...
# Authentication and security (commented out for now)
...
try:
    from maat_memory.memory import MaatMemory
    MAAT_MEMORY_AVAILABLE = True
except ImportError:
    MAAT_MEMORY_AVAILABLE = False
    MaatMemory = type(None)  # Use type(None) for type hints when not available
...
# Security middleware (order matters - apply first)
...
_vector_store = None
_embeddings = None
_maat_memory: Optional[Any] = None  # MaatMemory when available
...
async def query_rag(
    ...
    try:
        ...
        # Use full RAG query with LLM integration
        ...
        answer = rag_result.get("answer", "I couldn't process your question.")
        raw_sources = rag_result.get("sources", [])
        
        # Convert sources to API format
        sources = []
        for source in raw_sources:
            page_val = source.get("page", 0)
            try:
