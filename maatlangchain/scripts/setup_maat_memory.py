...
def configure_database_url():
    ...
    print_info("Or enter full URL, or enter each field separately")
    
    # Try full URL first
    full_url = input("Database URL (or press Enter for separate fields): ").strip()
    
    if full_url and full_url.startswith("postgresql://"):
        # User provided full URL
        db_url = full_url
    else:
        # Get fields separately
        if full_url:
            # User might have entered just hostname
            host = full_url or "localhost"
        else:
